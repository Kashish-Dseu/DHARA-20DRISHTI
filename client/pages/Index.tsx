...
import { SensorSnapshot, ReportsResponse, OverrideResponse } from "@shared/api";
...
export default function Index() {
  ...
  const trend = (sensors?.moistureTrend ?? []).map((v, i) => ({
    label: `D${i + 1}`,
    value: v,
  }));
  ...
  return (
    <div>
        ...
        <Card className="col-span-1">
          <CardHeader className="flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Last:{" "}
                {sensors?.lastIrrigation
                  ? new Date(sensors.lastIrrigation).toLocaleString()
                  : "--"}
              </CardDescription>
            </div>
          ...
          <CardContent className="flex gap-2">
            <Button
              className="flex-1"
              onClick={() => overrideMutation.mutate({ action: "start" })}
            >
            ...
            <Button
              variant="secondary"
              className="flex-1"
              onClick={() => overrideMutation.mutate({ action: "stop" })}
            >
              <Pause className="mr-2" /> Stop
            </Button>
            <DelayDialog
              onDelay={(m) =>
                overrideMutation.mutate({ action: "delay", delayMinutes: m })
              }
            />
          </CardContent>
        </Card>
        ...
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <LineChartIcon className="text-primary" /> Moisture Trend
            </CardTitle>
          </CardHeader>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mt-4 md:mt-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Automated schedule based on crop and weather
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mt-4 md:mt-6">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Water/energy savings and yield benefits
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <footer className="mt-10 text-xs text-muted-foreground">
        © {new Date().getFullYear()} DharaDrishti — Smart, renewable-first
        irrigation.
      </footer>
    </div>
  );
    ...
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Choose how many minutes to postpone the next event.
          </DialogDescription>
        </DialogHeader>
